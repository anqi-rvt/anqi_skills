...
|---|---|---|
| Text, headers, bold/italic/strikethrough | GFM headings (`#`–`####`, 4 levels); `**bold**`, `*italic*`, `~~strikethrough~~` | (none) |
...
| In-place rendered image, incl. animated GIF | Upload via `attachments.createUpload`, then `![alt](attachment-url)` | (none) |
...
`contentType`, and `size`, with no checksum/hash field and no
`duplicate_of`-style response. Left alone, re-running the sync script on an
unchanged image would upload it again every time, producing an attachment
graveyard.
...
position/text field of its own. Every `documents.update` call, including
one that pushes back byte-identical content, unconditionally detaches this
mark. The underlying `Comment` record is never dropped (threading and
content survive); only its inline position changes: `anchorText` comes
back `null` on the next fetch, regardless of whether the sync's content
